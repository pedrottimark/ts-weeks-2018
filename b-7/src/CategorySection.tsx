...
const getRecommendationClass = (recommendation: number) =>
    recommendation < 0 ? 'avoid' : recommendation === 0 ? 'limit' : ''

const getVitaminLink = ({ id, name, examples }: Vitamin, foodId: string) =>
...
const getMineralLink = ({ id, name, examples }: Mineral, foodId: string) =>
    examples.indexOf(foodId) === -1 ? (
        '\u00b7' // middle dot
    ) : (
        <Link to={`/minerals/${id}`}>
...
const CategorySection = ({ category, foods, minerals, vitamins }: Props) => (
    <section>
        <table>
            <colgroup>
                <col />
                ...
                {foods.map(({ id: foodId, recommendation }) => (
                    <tr key={foodId}>
                        ...
                        {vitamins.map(vitamin => (
                            <td key={`vitamins/${vitamin.id}`}>{getVitaminLink(vitamin, foodId)}</td>
                        ))}
                        {minerals.map(mineral => (
                            <td key={`minerals/${mineral.id}`}>{getMineralLink(mineral, foodId)}</td>
                        ))}
                    </tr>
                ))}
            </tbody>
        ...
        <aside>
            <img src={`${cdn}${pathnames[category]}`} alt={category} />
            <p>
